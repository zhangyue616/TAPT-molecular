...
class CMPNEncoder(nn.Module):
    ...
    def forward(self, step, mol_graph, features_batch=None) -> Tuple[torch.FloatTensor, torch.FloatTensor, List]:
        """
        前向传播（修改版：返回图向量、原子特征、原子范围）

        Returns:
            mol_vecs: [batch_size, hidden_size] - 图级别向量
            atom_hiddens: [num_atoms, hidden_size] - 原子级别特征
            a_scope: List[(start, size)] - 每个分子的原子范围
        """
...
class CMPN(nn.Module):
    ...
    def forward(self, step, prompt: bool, batch,
        ...
        """
        前向传播（修改版：透传编码器的三个返回值）

        Returns:
            如果 TAPT 模式：(mol_vecs, atom_hiddens, a_scope)
            否则：mol_vecs
        """
